...
const EndingScreen: React.FC<EndingScreenProps> = ({ onRestart }) => {
  ...
  const stats = [
    { icon: <Shield className="h-5 w-5" />, label: 'Biomes Restored', value: `${biomesCleared}`, color: 'text-glow-teal' },
    { icon: <Star className="h-5 w-5" />, label: 'Level Reached', value: `${state.level}`, color: 'text-primary' },
    { icon: <Heart className="h-5 w-5" />, label: 'Monsters Vanquished', value: `${monstersDefeated}`, color: 'text-glow-rose' },
    { icon: <BookOpen className="h-5 w-5" />, label: 'Compendium', value: `${compendiumUnlocked}/${compendiumTotal}`, color: 'text-glow-violet' },
    { icon: <Sparkles className="h-5 w-5" />, label: 'Estra Bond', value: `${state.estraBond}/5`, color: 'text-estra' },
  ];
        ...
        <AnimatePresence mode="wait">
          ...
          {(phase === 'title' || phase === 'stats' || phase === 'message' || phase === 'credits') && (
            ...
            >
              ...
              <h1 className="font-display text-4xl md:text-6xl text-primary text-glow-amber mb-3">
                The Bloom Awakens
              </h1>
              <motion.p
                className="text-lg md:text-xl text-foreground/80 max-w-md mx-auto leading-relaxed"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: 0.8, duration: 1 }}
              >
                {state.character?.name || 'Lyra'}'s journey has restored balance to the realm
              </motion.p>
            </motion.div>
          )}
          ...
          {(phase === 'stats' || phase === 'message' || phase === 'credits') && (
            <motion.div
              key="stats-block"
              className="absolute top-[42%] w-full max-w-lg px-4"
              initial={{ opacity: 0 }}
              animate={{ opacity: phase === 'stats' ? 1 : 0.7 }}
            ...
            >
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {stats.map((stat, i) => (
                  <motion.div
                    key={stat.label}
                    className="bg-black/40 backdrop-blur-sm border border-white/10 rounded-xl p-3"
          ...
          {(phase === 'message' || phase === 'credits') && (
            <motion.div
              key="message-block"
              className="absolute bottom-[22%] max-w-xl px-6"
            ...
            >
              <div className="bg-black/50 backdrop-blur-md border border-primary/20 rounded-2xl p-6">
                <p className="text-foreground/90 text-sm md:text-base leading-relaxed italic">
                  "Menopause is not an ending — it is a transformation. Every hot flash weathered,
                  every fog navigated, every myth shattered has made you stronger. The knowledge you carry
                  is your greatest remedy. You are not alone on this path."
                </p>
                <p className="text-primary/80 text-xs mt-3 font-display">— Dr. Mira, Hearth Village</p>
              </div>
            </motion.div>
          )}

          {/* Phase 4: Credits / Actions */}
          {phase === 'credits' && (
            <motion.div
              key="credits-block"
            ...
            >
              ...
              <p className="text-white/40 text-xs">Thank you for playing The M-QUEST</p>
              ...
              <div className="flex gap-3">
                ...
                >
                  <RotateCcw className="h-4 w-4 mr-1" /> New Journey
                </Button>
                <Button
                  size="sm"
                  onClick={onRestart}
                ...
                >
                  <Sparkles className="h-4 w-4 mr-1" /> Continue Exploring
                </Button>
              </div>
            </motion.div>
          )}
