...
const AudioControls = ({
  ...
  return (
    <div className="relative">
      <Button variant="ghost" size="sm" onClick={toggleMute} className="relative"
        title={muted ? 'Unmute' : 'Mute'}>
        {muted ? <VolumeX className="h-4 w-4" /> : <Volume2 className="h-4 w-4" />}
      </Button>
      <Button variant="ghost" size="sm" onClick={() => setOpen(!open)} title={t('audio.settings')}>
          ...
          >
            <div className="flex items-center justify-between text-xs font-display text-primary mb-1">
              ...
              <Button variant="ghost" size="sm" className="h-6 px-2 text-xs" onClick={playChime}>
                <Sparkles className="h-3 w-3 mr-1" /> {t('audio.test')}
              </Button>
            </div>
            ...
            <div className="space-y-1">
              ...
              <Slider value={[masterVolume * 100]} onValueChange={([v]) => setMasterVolume(v / 100)} max={100} step={1} className="w-full" />
            </div>
            ...
            <div className="space-y-1">
              ...
              <Slider value={[musicVolume * 100]} onValueChange={([v]) => setMusicVolume(v / 100)} max={100} step={1} className="w-full" />
            </div>
            ...
            <div className="space-y-1">
              ...
              <Slider value={[sfxVolume * 100]} onValueChange={([v]) => setSfxVolume(v / 100)} max={100} step={1} className="w-full" />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
